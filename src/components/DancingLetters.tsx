...
export const DancingLetters = ({
  text = "Tech Compensation Predictor",
  className = "",
  letterClassName = "",
}: DancingLettersProps) => {
  ...
  return (
    <LazyMotion features={domAnimation}>
      <m.div
        className={cn(
          "flex items-center justify-center select-none flex-wrap gap-x-1",
          className,
        )}
      ...
      >
        {letters.map((letter, id) => {
          if (letter === " ") {
            return (
              <span key={`space-${id}`} className="inline-block w-4 md:w-6">
                &nbsp;
              </span>
            );
          }
          ...
          return (
            <m.span
              ...
              onHoverStart={() => {
                setActiveIndices((prev) => new Set(prev).add(id));
              }}
              onHoverEnd={() => {
                setActiveIndices((prev) => {
                  const next = new Set(prev);
              ...
              className={cn(
                "relative inline-block text-4xl md:text-5xl lg:text-6xl font-extrabold text-white cursor-pointer tracking-tight transition-colors duration-200 hover:text-sky-400",
                letterClassName,
                isActive ? "z-10" : "z-0",
              )}
              style={{
